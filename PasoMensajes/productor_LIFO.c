...
void productor() {
  ...
  unsigned int prioridad=0; //Variable utilizada para asignar prioridad a los mensajes al enviarlos
  ...
  for(i=0;i<DATOS_A_PRODUCIR;i++){ //Realizamos tantas iteraciones como elementos se quieren producir
    ...
    mq_send(almacen2,mensaje,sizeof(mensaje),prioridad); //Enviamos el mensaje al consumidor
    printf("Item producido: %s Prioridad: %d\n",mensaje,prioridad); //Mostramos el item producido junto con la prioridad con la que es enviado
    prioridad++; //Aumentamos el valor de prioridad para que sea siempre creciente
  }
